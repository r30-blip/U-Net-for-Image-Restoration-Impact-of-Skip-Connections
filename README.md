...
本研究で使用した元画像データはデータ量が膨大であるため、
本リポジトリには含めていません。

本コードを利用する際は、**ご自身のデータセット**を用いてください。

---

## 概要

本プロジェクトでは、**U-Net アーキテクチャを用いた劣化画像復元**を目的とし、  
CIFAR-10（32×32 カラー画像）に対して人工的に強い劣化処理を施したデータセットを用いて実験を行いました。

Encoder–Decoder 構造に **skip connection を導入した U-Net** と、  
**skip connection を持たない Enc–Dec モデル**を比較し、  
復元性能および構造的な違いが結果に与える影響を検証しています。

---

## データセットと劣化処理

- **劣化処理スクリプト**：`cifar10_degradation.m`
- **元データ**：CIFAR-10（32×32 RGB 画像）

以下の劣化処理を **複合的** に適用しました。
...
### 1. Encoder–Decoder（no-skip）モデル

- **ファイル名**：`no_skip.m`
- 畳み込み + MaxPooling による Encoder
- 転置畳み込みによる Decoder
- Encoder で圧縮された特徴量は Decoder に直接渡されない
...
強い複合劣化が施された画像では、  
Encoder 内で圧縮された際に **輪郭や局所構造の情報が失われやすい**ことが確認されました。

no-skip モデルでは、この情報損失を Decoder 側で十分に補えず、  
全体的にぼやけた復元結果となりました。

一方、U-Net では skip connection を通じて  
Encoder の浅い層が保持する **高解像度かつ局所的な特徴**を Decoder に直接伝達できるため、  
輪郭や形状が明瞭な復元結果が得られました。

このことから、  
**強い劣化を伴う画像復元問題において U-Net 構造は有効である**ことが確認されました。
